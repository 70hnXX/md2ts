...
export const generator = (config: InterfaceConf) => {
  const { title, method, url, functionName, reqParams, reqData, resBody } = config;
  ...
  if (isGet) {
    for (const key in reqParams) {
      const isLast = key === Object.keys(reqParams)[Object.keys(reqParams).length - 1];
    ...
    for (const key in reqData) {
      const isLast = key === Object.keys(reqData)[Object.keys(reqData).length - 1];
  ...
  for (const key in resBody) {
    const isLast = key === Object.keys(resBody)[Object.keys(resBody).length - 1];
